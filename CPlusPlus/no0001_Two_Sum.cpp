...
class Solution { // define class Solution
public: // public members
	// Two Sum: return indices i, j such that nums[i] + nums[j] == target
	std::vector<int> twoSum(std::vector<int>& nums, int target) { // input array by reference and target sum
		std::unordered_map<int, int> seen; // map value -> index for numbers we've seen
		for (int i = 0; i < (int)nums.size(); ++i) { // iterate indices from left to right
			...
			if (it != seen.end()) {            // if found, we have a valid pair
...
static void printVec(const std::vector<int>& v) { // helper: print vector as [a,b,c]
	std::cout << "[";                            // opening bracket
	for (size_t i = 0; i < v.size(); ++i) {       // print each element
		std::cout << v[i] << (i + 1 == v.size() ? "" : ","); // comma between elements, none after last
	}
	std::cout << "]\n";                         // closing bracket and newline
}

int main() {                               // entry point: simple local tests
